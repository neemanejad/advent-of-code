...
std::unordered_map<std::string, Operation> operation_map
    = { { "AND", Operation::AND },       { "OR", Operation::OR },
        { "LSHIFT", Operation::LSHIFT }, { "RSHIFT", Operation::RSHIFT },
        { "NOT", Operation::NOT },       { "VALUE", Operation::VALUE } };
...
template <class T>
void
set_optional_value (Optional<T> &optional, T value)
{
...
bool
can_use_value (std::string str,
               const std::unordered_map<std::string, Wire> &wires)
{
    if (is_number (str))
        return true;
...
int
get_value (std::string str, const std::unordered_map<std::string, Wire> &wires)
{
    if (is_number (str))
...
void
get_a (std::unordered_map<std::string, Wire> &wires)
{
    ...
    while (stack.size () != 0)
    {
        ...
        if (wire->operation == Operation::VALUE)
        {
            ...
            {
                set_optional_value (wire->value,
                                    get_value (wire->input[0], wires));
            }
            else
            {
        ...
        else
        {
            ...
            case Operation::AND:
            {
                ...
                // current wire back to stack then add each input wire as
                // needed to the stack
                auto wire1 = wire->input[0];
                auto wire2 = wire->input[1];
                if (can_use_value (wire1, wires)
                    && can_use_value (wire2, wires))
                {
                    set_optional_value (wire->value,
                                        get_value (wire1, wires)
                                            & get_value (wire2, wires));
                }
                else
                {
                    stack.push (wire->name);
            ...
            default:
            {
                throw new std::invalid_argument (
                    "Invalid operation that requires two wires");
            }
            }
        }
    }
...
main (void)
{
    ...
    while (getline (input_file, buffer))
    {
        ...
        else if (operation_map.find (words[0]) != operation_map.end ()
                 && operation_map[words[0]] == Operation::NOT)
        {
            wire.name = words[words.size () - 1];
    ...
    // Display "a"
    std::cout << wires["a"].name << " has value of " << wires["a"].value.value
              << std::endl;

    // ----------- Part 2 -----------

    // Overwrite "b" with "a"
    wires.at ("b").value.value = wires.at ("a").value.value;

    // Invalidate all wire values except for "b"
    for (auto wire = wires.begin (); wire != wires.end (); wire++)
    {
        if (wire->second.name == "b")
            continue;

        wire->second.value.is_set = false;
    }
